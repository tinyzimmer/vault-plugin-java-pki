...
func certBundleToKeystoreChain(cb *certutil.CertBundle) ([]keystore.Certificate, []keystore.Certificate) {
	certificates := make([]keystore.Certificate, 0)
	trustedCertificates := make([]keystore.Certificate, 0)
	for _, cert := range cb.CAChain {
		if len(cert) > 0 {
			trustedCertificates = append(trustedCertificates, keystore.Certificate{
				Type:    "X.509",
				Content: []byte(cert),
			})
		}
	...
	certificates = append(certificates, keystore.Certificate{
		Type:    "X.509",
		Content: []byte(cb.Certificate),
	})
	return certificates, trustedCertificates
}
...
func EncodePEMToJKS(pb *certutil.ParsedCertBundle, cb *certutil.CertBundle, passw string) (string, error) {
	...
	for idx, x := range trusted {
		jks[fmt.Sprintf("trusted_%v", idx)] = &keystore.TrustedCertificateEntry{
			Entry: keystore.Entry{
				CreationDate: time.Now(),
			},
			Certificate: x,
	...
	err = keystore.Encode(&out, jks, []byte(passw))
	if err != nil {
		return "", err
	}
